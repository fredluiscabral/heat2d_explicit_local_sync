...
// OpenMP com semáforos POSIX e mitigação de false sharing:
// semáforos padded, campos alinhados/padded e first-touch paralelo.
...
static_assert(sizeof(sem_t) <= heat2d::CACHELINE_BYTES,
              "Esta implementação assume sem_t cabendo em uma cache line.");
...
int main() {
    ...
    for (int t = 0; t < max_threads; ++t) {
        sem_init(&sem_left[static_cast<std::size_t>(t)].value, 0, 0);
        sem_init(&sem_right[static_cast<std::size_t>(t)].value, 0, 0);
    }
    ...
    #pragma omp parallel default(shared)
    {
        ...
        auto wait_neighbors = [&]() {
            if (tid > 0)      sem_wait(&sem_right[static_cast<std::size_t>(tid - 1)].value);
            if (tid + 1 < nt) sem_wait(&sem_left[static_cast<std::size_t>(tid + 1)].value);
        };
    ...
    heat2d::print_summary("omp_semaforos_nofs", p, dt, lam, secs, err);
